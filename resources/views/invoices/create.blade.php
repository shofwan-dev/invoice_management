<x-app-layout>
    ...
    <div class="py-6">
        <div class="container-lg">
            <div class="card shadow-sm border-0">
                <div class="card-body p-6">    <!-- Debug Info -->
        ...
        @endif
                    ...
                    <div class="row mb-3">
                        <div class="col-lg-6">
                            ...
                            <input type="date" name="invoice_date" class="form-control shadow-sm @error('invoice_date') is-invalid @enderror" value="{{ old('invoice_date', $invoice->invoice_date ?? today()->toDateString()) }}" required>
                            @error('invoice_date')
                        ...
                        <div class="col-lg-6">
                            ...
                            <input type="date" name="payment_deadline" class="form-control shadow-sm @error('payment_deadline') is-invalid @enderror" value="{{ old('payment_deadline', $invoice->payment_deadline ?? '') }}">
                    ...
                    <div class="row mb-3">
                        <div class="col-lg-6">
                            <label class="form-label">Nama yang Ditagih (Client)</label>
                            <input type="text" name="client_name" class="form-control shadow-sm @error('client_name') is-invalid @enderror" value="{{ old('client_name', $invoice->client_name ?? '') }}" placeholder="Nama klien" required>
                            @error('client_name')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <!-- Rincian Tagihan -->
                    <h5 class="mb-3 border-bottom pb-2">Rincian Tagihan</h5>
                    <div class="table-responsive mb-3">
                        <table class="table table-bordered table-hover" id="items_table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td><input name="items[{{ $i }}][description]" class="form-control form-control-sm shadow-sm" value="{{ $it['description'] ?? '' }}" placeholder="Deskripsi item" required></td>
                                    <td><input type="number" min="1" name="items[{{ $i }}][qty]" class="form-control form-control-sm shadow-sm qty" value="{{ $it['qty'] ?? 1 }}" required></td>
                                    <td>
                                        <input type="text" 
                                               name="items[{{ $i }}][price]" 
                    ...
                    <!-- Tahapan Pembayaran -->
                    <h5 class="mb-3 border-bottom pb-2">Tahapan Pembayaran</h5>
                    <div class="table-responsive mb-3">
                        <table class="table table-bordered table-hover" id="payment_table">
                            <thead class="table-light">
                                <tr>
                                    <th style="width: 80px;">Pembayaran ke</th>
                                    <th>Jumlah</th>
                            ...
                            <tbody id="payment_steps">
                                @php $psOld = old('payment_steps', isset($invoice) ? $invoice->paymentSteps->toArray() : []); @endphp
                                @forelse($psOld as $i => $ps)
                                <tr class="payment-row">
                                    <td><input type="number" min="1" name="payment_steps[{{ $i }}][step_number]" class="form-control form-control-sm shadow-sm" value="{{ $ps['step_number'] ?? 1 }}" placeholder="1" required></td>
                                    <td>
                                        <input type="text" 
                                    ...
                                    <td><input type="date" name="payment_steps[{{ $i }}][payment_date]" class="form-control form-control-sm shadow-sm" value="{{ $ps['payment_date'] ?? '' }}"></td>
                                    <td><input type="text" name="payment_steps[{{ $i }}][bank_name]" class="form-control form-control-sm shadow-sm" value="{{ $ps['bank_name'] ?? '' }}" placeholder="Nama Bank"></td>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('Invoice form calculation script loaded - FIXED VERSION');
        ...
        function addItemHandler() {
            console.log('Add item clicked - FIXED');
            ...
            newRow.innerHTML = `
                ...
                <td><input type="text" name="items[${rowCount}][price]" class="form-control form-control-sm shadow-sm price" value="0" placeholder="0" required></td>
        ...
        function addPaymentHandler() {
            console.log('Add payment clicked - FIXED');
            ...
            newRow.innerHTML = `
                ...
                <td><input type="text" name="payment_steps[${rowCount}][amount]" class="form-control form-control-sm shadow-sm payment-amount" value="0" placeholder="0" required></td>
        ...
        function setupEventListeners() {
            console.log('Setting up event listeners - FIXED');
        ...
        setTimeout(() => {
            setupEventListeners();
            updateAllCalculations();
            console.log('Invoice form calculations initialized - FIXED VERSION');
        }, 100);
    });
    </script>
</x-app-layout>